...
// Hardening v3 F3: merge in externally-built golden fixtures (from the
// ingest pipeline). golden-data/index.ts is generated by
// scripts/golden/build.ts --import-index; when empty (no real annotated data
// yet) the array is empty and GOLDEN_FIXTURES stays just the curated inline
// set. A static import is used so the compiler always sees the module.
import { EXTERNAL_GOLDEN_FIXTURES } from '../../../golden-data/index';
export const ALL_GOLDEN_FIXTURES: GoldenFixture[] = [
  ...GOLDEN_FIXTURES,
  ...EXTERNAL_GOLDEN_FIXTURES,
];
